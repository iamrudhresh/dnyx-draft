...
export function ConfigViewer({ content, filename }: ConfigViewerProps) {
  ...
  useEffect(() => {
    if (ext === 'toml') {
      ...
      const dynamicImport = new Function('m', 'return import(m)') as (m: string) => Promise<{ parse: (s: string) => Record<string, unknown> }>;
      dynamicImport('@iarna/toml').then((toml) => {
        try {
        ...
        setRows([{ key: 'note', value: '@iarna/toml not installed — add it to package.json to parse .toml files' }]);
      });
    } else {
    ...
    <table className="w-full text-sm border-collapse">
      <thead>
        <tr className="bg-slate-50 dark:bg-slate-800">
          <th className="text-left px-3 py-2 border border-slate-200 dark:border-slate-700 font-semibold text-slate-700 dark:text-slate-300 w-1/3">Key</th>
          <th className="text-left px-3 py-2 border border-slate-200 dark:border-slate-700 font-semibold text-slate-700 dark:text-slate-300">Value</th>
        </tr>
      </thead>
      <tbody>
        {items.map((row, i) => (
          <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
            <td className="px-3 py-1.5 border border-slate-200 dark:border-slate-700 font-mono text-blue-700 dark:text-blue-400">{row.key}</td>
            <td className="px-3 py-1.5 border border-slate-200 dark:border-slate-700 font-mono text-slate-700 dark:text-slate-300 break-all">{row.value}</td>
          </tr>
        ))}
      </tbody>
    </table>
  ...
  return (
    <div className="h-full overflow-auto p-6 space-y-6">
      ...
      {sections.map((section) => (
        <div key={section}>
          <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-2">[{section}]</h3>
