<div align="center">
  ...
  <img src="../assets/icon.jpg" alt="Dnyx Draft 徽标" width="100" />

  <h1>Dnyx Draft</h1>

  **提供实时预览的本地优先 Markdown 编辑器与查看器**

  在浏览器、渐进式 Web 应用（PWA）或 Docker 中打开、编写、整理、审阅、渲染和导出 Markdown；需要时还可使用分享与协作功能。

  [![许可证](https://img.shields.io/github/license/dnyxtech/dnyx-draft?style=flat-square&color=red)](../LICENSE)
  [![最新版本](https://img.shields.io/github/v/release/dnyxtech/dnyx-draft?style=flat-square&color=FF6B00)](https://github.com/dnyxtech/dnyx-draft/releases)

  <p>
    <a href="https://codewiki.google/github.com/dnyxtech/dnyx-draft" target="_blank" rel="noopener noreferrer">
      <img src="https://img.shields.io/badge/CodeWiki-Explore-4285F4?logo=wikipedia&logoColor=white&style=flat" alt="在 CodeWiki 上探索 Dnyx Draft" />
    </a>
    <a href="https://deepwiki.com/dnyxtech/dnyx-draft" target="_blank" rel="noopener noreferrer">
      <img src="https://deepwiki.com/badge.svg" alt="在 DeepWiki 上探索 Dnyx Draft" />
    </a>
  </p>

  [English](../README.md) · [日本語](README_ja.md) · [한국어](README_ko.md) · **简体中文** · [繁體中文](README_tw.md)

  [打开 Web 应用](https://draft.dnyxgroup.com/) · [文档（英文）](../wiki/Home.md) · [报告问题](https://github.com/dnyxtech/dnyx-draft/issues) · [版本发布](https://github.com/dnyxtech/dnyx-draft/releases)
...
Dnyx Draft 是一个开源、本地优先的工作区，适合开发者、写作者、学生、研究人员以及所有使用 `.md` 或 `.markdown` 文件的用户。它不止于纯文本：在一个专注的工作区中整理多个文档，在不改动 Markdown 的情况下通过评论和建议进行审阅，并渲染图表、地图、数据图、数学公式、3D 模型和音乐。

将文档转换为 Share Snapshot 链接以便快速分享，或启动带访问控制的 Live Share，进行实时协同编辑并使用实时光标、评论和建议。日常编辑在设备上完成。无需账号，应用也不包含广告、分析、遥测或订阅。

<p align="center">
  <img src="https://github.com/user-attachments/assets/5a0d6fda-96f0-4baf-bf7a-0ffbe5119eab" alt="Dnyx Draft 应用界面" width="90%" />
</p>

## 核心功能

- **工作区与文档：** 在 Web 端使用按文档划分的 IndexedDB 存储，并在嵌套文件夹中整理文档。还可使用最近使用、收藏夹、搜索、标签页、批量操作和加密的 Secret Workspace（秘密工作区）。
- **备份与恢复：** 导出或导入保留文件夹结构的工作区 ZIP。可以选择包含加密的 Secret Workspace 文件，但回收站不包含在备份中。
...
- **多种运行方式：** 使用托管 Web 应用、PWA、静态托管、Docker 或 Cloudflare。

有关已实现的行为和限制，请参阅[功能参考（英文）](../wiki/Features.md)。

## 快速开始

直接使用[托管 Web 应用](https://draft.dnyxgroup.com/)，或通过本地 HTTP 服务器运行仓库：
...
Dnyx Draft 以本地处理为优先，但并非所有功能都能离线运行。
...
以下基于 Dnyx Draft 的社区项目由其开发者独立维护。

- [**Markdown Desk**](https://github.com/jhrepo/markdown-desk)：使用 Tauri 构建的原生 macOS 封装应用，增加了原生文件对话框与文件处理程序、菜单栏集成、自动重新加载和应用内更新功能。

## 开发历程

Dnyx Draft 最初是 PC 上的一个小型个人项目：一款源于好奇心、经历过错误与修正，并倾注了许多心力的简单 Markdown 查看器。初始版本始终是这个项目的初心所在。

如需了解更完整的项目历史，请阅读 [Development Journey（英文）](../wiki/Development-Journey.md)。

## 贡献者

Dnyx Draft 在社区贡献者的共同参与下不断成长。

<a href="https://github.com/dnyxtech/dnyx-draft/graphs/contributors" target="_blank" rel="noopener noreferrer">
  <img src="https://contrib.rocks/image?repo=dnyxtech/dnyx-draft" alt="Dnyx Draft 贡献者" />
</a>

## 参与贡献与支持

创建 Pull Request 前，请阅读 [Contributing（英文）](../wiki/Contributing.md)。可复现的缺陷和明确的功能建议可提交至 [Issue Tracker](https://github.com/dnyxtech/dnyx-draft/issues)。

请勿在普通 Issue 中发布漏洞详情。请使用 [Contributing: Security Reports（英文）](../wiki/Contributing.md#security-reports)中说明的仓库私密安全报告渠道。

## 许可证

Dnyx Draft 采用 [Apache License 2.0](../LICENSE)。
