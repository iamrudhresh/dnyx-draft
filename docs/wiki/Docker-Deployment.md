# Docker Deployment for the Dnyx Draft Web App

Dnyx Draft's Docker image serves the browser-based Markdown editor and viewer as a static web app with Nginx. It is useful for local networks, self-hosting, and reverse-proxy deployments.
...
```bash
docker build -t dnyx-draft:local .
docker run -d --name dnyx-draft-local -p 8080:80 dnyx-draft:local
```

The root `.dockerignore` excludes unrelated local files from the web container.
...
On `localhost`, managed-media and stored Share Snapshot requests use the production `https://draft.dnyxgroup.com` API. Live Share defaults to the Docker origin and fails unless a compatible `/live-room` endpoint is available or `window.DNYX_DRAFT_LIVE_ROOM_URL` is configured in a custom deployment.
...
- The stock localhost client sends consented managed-media and stored Share Snapshot requests to the production Dnyx Draft API.
