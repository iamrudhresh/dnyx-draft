# Dnyx Draft Features: Live Markdown Preview, Diagrams, Export, and Sharing

This page is the source-of-truth feature reference for Dnyx Draft. It describes what the app does, how each feature behaves for users, the implementation limits that matter in practice, and what happens to document data.

## Product Summary

Dnyx Draft is a browser-based Markdown editor and viewer for opening `.md` and `.markdown` files, writing plain Markdown, and reading a live GitHub-style Preview. It runs as a static web application, a Progressive Web App, and a Docker-hosted static site. The Editor is built around a plain textarea, a rendered Preview pane in Split view with sync scrolling, Document tabs, import/export tools, sharing tools, rich Markdown renderers, and optional Cloudflare endpoints for Share Snapshot and Live Share.

Most work happens in the browser. Markdown parsing, syntax highlighting, math rendering, diagram post-processing, PDF/PNG capture, tab storage, undo/redo, search, and formatting tools are client-side. The exceptions are explicit network features: GitHub import, emoji lookup, CDN library loading, remote diagram fallback services, large Share Snapshot storage, and Live Share relay rooms.
...
- The sidebar is resizable and collapsible on larger screens, narrower on tablet, and becomes a full-height drawer on mobile.
...
Document data lives in IndexedDB while small preferences remain in `localStorage`. Existing monolithic `markdownViewerTabs` data is migrated once.

Workspace settings includes **Private mode**, which pauses document-state writes for the current private session without clearing existing documents or Secret Workspace. The private-mode preference remains so the behavior survives a reload. **Storage and Backup** reports usage, separate normal and secret document counts, a logical browser storage location, exports/imports ZIP backups, and can include unchanged encrypted Secret Workspace records. Import permanently replaces the current workspace after confirmation. Backups exclude trash and recovery journals. Browser storage is best-effort by default and is shown as read-only status. **Reset workspace** permanently clears all local workspace data after confirmation; **Reset Secret Workspace** remains available for deleting only the encrypted area.
...
- Open **Review** from the document toolbar or mobile menu.
...
- The panel is a side panel on larger screens, a drawer on tablet, and a touch-friendly bottom sheet on mobile, using the app's existing colors, controls, themes, and accessibility patterns.
...
- Find and Replace and Fullscreen are direct formatting-toolbar actions. About Dnyx Draft opens from the header or mobile menu.
...
Dnyx Draft supports many fenced-code renderers for diagrams, charts, mind maps, digital timing diagrams, music notation, geographic maps, and 3D STL models.
...
Dnyx Draft can open `.md` and `.markdown` documents from local files, drag and drop, and GitHub URLs.
...
- Local file content is read in the browser and is not uploaded by local import.
...
- A private-repository PAT is attached only to `api.github.com` requests and is never sent to `raw.githubusercontent.com` or the Dnyx Draft backend. Local deletion does not revoke the token on GitHub.
...
- The Share API allows CORS for the production app, HTTPS `*.dnyx-draft.pages.dev` previews, `null`, and `localhost`/`127.0.0.1` development origins; unsupported browser origins are rejected.
...
- The Pages Function and Durable Object reject unsupported WebSocket `Origin` values. The production app, HTTPS `*.dnyx-draft.pages.dev` previews, `null`, and localhost development origins are allowed.
...
| Normal tab autosave | No | Per-document IndexedDB records | Content is written independently rather than serializing the whole Workspace. |
...
Dnyx Draft opens and displays more than just Markdown. Dropping or importing any supported file type opens it in a dedicated native viewer rather than a text editor. Non-markdown documents appear in the same tab system and are stored in the same workspace.

### Supported File Types

| Extension(s) | Viewer | Notes |
| :--- | :--- | :--- |
| `.md`, `.markdown`, `.txt` | Markdown editor + live preview | Full editor and all formatting tools |
| `.docx` | Converts to Markdown | Uses mammoth + turndown; conversion notes shown in import dialog |
| `.html`, `.htm` | Converts to Markdown | Uses turndown |
| `.pdf` (imported as text) | Converts to Markdown (best-effort) | Born-digital PDFs only; scanned PDFs are not supported |
| `.pdf` (imported as file) | PDF viewer | Canvas-rendered pages, page navigation, in-document search |
| `.csv`, `.tsv` | Table viewer | Sortable columns, editable cells, insert-as-chart action, Format/Minify/Validate/Convert toolbar |
| `.json` | Tree viewer | Collapsible nodes, editable values, live AJV schema validation, Format/Minify/Validate/Convert/Escape toolbar |
| `.yaml`, `.yml` | Tree viewer | Shares the same tree UI as the JSON viewer, plus its own Format/Minify/Validate/Convert toolbar |
| `.ipynb` | Notebook viewer | Renders markdown and code cells; outputs include plain text, HTML, and base64 images |
| `.xlsx`, `.xls` | Spreadsheet viewer | Sheet-tab selector for multi-sheet workbooks |
| `.xml` | Tree viewer | Parsed with the browser's native DOMParser; fully editable with a Format/Minify/Validate/Convert/Escape toolbar |
| `.env`, `.ini`, `.toml` | Config viewer | Key-value table; TOML requires `@iarna/toml` to be installed |
| `.zip` | Archive browser | Browsable file tree; click any entry to open it in the correct viewer |
| `.js`, `.ts`, `.jsx`, `.tsx`, `.py`, `.rb`, `.go`, `.rs`, `.java`, `.cpp`, `.c`, `.cs`, `.php`, `.swift`, `.sh`, `.css`, `.sql` | Code viewer | Syntax-highlighted read-only view with copy button |
| `.jpg`, `.jpeg`, `.png`, `.gif`, `.webp`, `.svg`, `.bmp`, `.ico` | Image viewer | Centered display with zoom and fit-to-pane controls |

### File Import

Open **Import File…** from the file menu or drag files directly onto the drop zone.

- **Single file**: drag and drop or click **Browse Files**.
- **Folder**: click **Import Folder** to import an entire directory at once. Each supported file is imported; unsupported files are skipped.
- **Multiple files**: select several files at once; a progress indicator tracks each one.
- **DOCX and HTML**: converted to Markdown at import time. Conversion notes (e.g. unsupported styles) appear in a collapsible panel.
- **PDF, notebook, XLSX, CSV, JSON, YAML, code, image, XML, config, archive**: stored directly as their native type and opened in the appropriate viewer.
- Binary files (images, PDFs opened as files) are stored in the `blobs` IndexedDB table rather than as string content.

### Editable Viewers

**CSV / TSV**: click any cell to edit in place. Changes are written back to the document's stored content. Use the header context menu to add or remove columns.

**JSON**: click any primitive value to edit it inline. Add or remove object keys and array items. The document serializes back to `JSON.stringify` with two-space indentation on every change. If a `$schema` key is present, AJV validates the document live and highlights invalid fields.

**YAML**: the same inline editor as JSON. Serializes back via `js-yaml`'s `dump()`.

**XML**: the same inline tree editor as JSON. Edits serialize back to XML text, preserving attributes.

### Format, Convert & Escape Tooling

JSON, XML, YAML, and CSV viewers each show a toolbar above the document with five actions:

- **Format** — pretty-prints the document in place (2-space indent for JSON/CSV; `js-yaml`'s default block style for YAML; indented tags for XML).
- **Minify** — collapses the document to its most compact form in place.
- **Validate** — checks the document (parse success, plus AJV `$schema` validation for JSON, well-formedness for XML, and a column-count warning for CSV) and reports the result without changing the document.
- **Convert** — opens a dialog to convert the document to one of the other three formats, with a live preview. By default it creates a new document with the converted content; a **Replace Current File** button is also available. Converting a table (CSV) to a nested format flattens objects into `key.nested` columns; converting back expects a top-level array of flat objects.
- **Escape/Unescape** — transforms the current document text (JSON string escape, XML entity escape, URL encoding, or Base64) and copies the result to the clipboard. This does not change the document, since it is typically used on a snippet rather than the whole file.

Format and Minify write back to the document immediately, the same way editing a cell or tree value does. Validate and Escape/Unescape never modify the document.

### Export as JSON

`ExportModal` includes **Export as JSON** alongside Markdown, HTML, PDF, and PNG. This walks the parsed Markdown AST and produces structured JSON: `{ metadata: { title, tags, createdAt }, sections: [...] }` grouped by headings.

### Search Across All File Types

Workspace search covers all open documents. For types whose content is not a plain string, extracted text is stored in the `searchableText` field:

- **Notebooks (.ipynb)**: cell sources are extracted at import time.
- **PDFs**: pdfjs extracts text from born-digital PDFs in the background after the file is stored; scanned PDFs return no searchable text.
- **CSV, JSON, YAML, code**: the raw string content is searched directly.

### Diagram Generation from Data

The **Insert as Chart** button in `CsvTableViewer` generates a `vega-lite` fenced code block in a new Markdown document, pre-populated with your CSV data. The chart renders immediately in the Markdown preview using the existing VegaLiteViewer.

### AI-Assisted Authoring (BYOK)

Dnyx Draft supports AI writing assistance using your own API key. No key is ever sent to Dnyx Draft's servers.

**Setup — Settings → AI:**
1. Choose your provider: **Anthropic** (Claude) or **OpenAI** (GPT-4).
2. Enter a passphrase. This passphrase is used to derive the encryption key and is required once per session.
3. Enter your API key and click **Save**. The key is encrypted with AES-256-GCM (PBKDF2-SHA256, 100 000 iterations) and stored in IndexedDB. The decrypted key is kept in session memory only — it is never written to `localStorage` or re-stored.
4. Click **Test connection** to verify the key works before saving.
5. Click **Delete key** to remove the stored key permanently.

**EditorToolbar AI actions (Bot button):**

The button label shows "AI — acts on selection" when text is selected in the editor, or "AI — acts on document" when nothing is selected.

| Action | Behavior |
| :--- | :--- |
| Summarize | Summarizes selected text (or the full document) and inserts the result |
| Fix Grammar | Corrects grammar and phrasing in the selection or document |
| Make Concise | Rewrites the selection or document to be shorter |
| Generate Diagram | Opens a description prompt; generates Mermaid syntax and inserts a fenced `mermaid` code block |

For selection-based actions, the result is inserted at the cursor. For full-document actions, the document content is replaced. Diagram generation always inserts at the cursor regardless of selection.

**Data handling:** your text is sent directly to the provider API you configured. No Dnyx Draft server sees the content.

## Known Technical Limits

- Browser storage quotas can reject very large saved workspaces.
- Dnyx Draft does not impose a document-count limit; available storage and operating-system/filesystem constraints still apply.
