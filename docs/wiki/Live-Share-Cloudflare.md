...
Live Share is the temporary Markdown collaboration feature in Dnyx Draft. It is separate from Share Snapshot. Share Snapshot creates a point-in-time link; Live Share relays Yjs updates through a WebSocket room.
...
- Allows the production app, HTTPS `*.dnyx-draft.pages.dev` preview deployments, `null`, and localhost/127.0.0.1 development `Origin` values.
