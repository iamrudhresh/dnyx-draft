...
| Retention | No server-side record created by Dnyx Draft; the copied URL itself remains usable | 90 days |
...
> **Important:** The current Dnyx Draft UI does not display or export the deletion token and does not provide a Delete snapshot action. It keeps the token only in page memory while the generated link state exists. Closing/resetting the modal or leaving the session loses normal UI access to it.
...
- Allowed application origins include the production site, HTTPS `*.dnyx-draft.pages.dev` previews, `null`, and local `localhost`/`127.0.0.1` development origins.
