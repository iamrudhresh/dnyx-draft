...
This guide explains how to use Dnyx Draft as a browser-based Markdown editor, viewer, reader, and live Preview tool. For deeper implementation notes and renderer limits, see [Features](Features.md). For data handling, see [Privacy and Security](Privacy-and-Security.md).
...
- On larger screens, one click selects a sidebar document and a double click opens it. On touch layouts, one tap opens it and closes the drawer.
...
- Normal tabs autosave as independent browser records. Only the opened document content is loaded.
- Dnyx Draft does not impose a document-count limit; available browser quota is the practical limit.
...
- Use **Storage and Backup** to inspect usage and file counts, view the browser's read-only persistence status, and export or import a folder-preserving ZIP.
...
1. Click **Review** in the document toolbar or mobile menu.
...
The panel toolbar can copy a Markdown summary, resolve all open items, or delete all feedback after confirmation. Review uses a side panel on larger screens, a drawer on tablet, and a bottom sheet on mobile. Opening a new tab closes Review automatically.
...
| :--- | :--- | :--- |
| Markdown | Saves the raw `.md` text. | Browser downloads only. |
